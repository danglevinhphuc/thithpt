...
<?php
	...
	include "db.php";
  $query = "SELECT * FROM `users` WHERE user = '$username' and phone = '$phone' and email = '$email'";
  $sql = mysqli_query($conn,$query) or die ("Can connect to users");
  if(mysqli_num_rows($sql)){
    $query_1 = "UPDATE  users set password = '$password' where user = '$username' ";
    if(!mysqli_query($conn,$query_1)){
    die('<center><br><br><br>Can not connect .</center>');
    }
    echo "<center><br><br><br>Congratulation!<br>Đỗi tài khoản thành công.</center>";
  mysqli_close($conn);
  }
  else{
    echo "<center><br><br><br>Wrong!<br>Tài khoảng không tồn tại.</center>";
  }
